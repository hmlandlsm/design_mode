  ...
  <div class="home-page">
    ...
    <ul class="list" v-if="firstPageTabsFunds && firstPageTabsFunds.tabFunds && userMsg" @touchstart="touchstart" @touchend="touchend">
      <div class="tabs-title" v-if="showTabsFundModel">
        ...
        <div class="risk-tip" v-if="!showAllFund && userMsg && userMsg.riskLevel < 5">
        已根据您的风险承受能力筛选产品，点击
    ...
    <scroll-view class="water_fall" scroll-y @scroll="waterFallScroll">
        <div class="water_box" ref="scrollContainer">
            <div class="water_col mgr20">
                <div class="item"  ref="leftWaterItem" v-for="(item, index) in leftPosterList" @click="goLink(item.link, `资讯跳转`)">
                    <img :src="item.img"
                    @load="imgLoaded(0)"
                    class="img"
                    lazy-load="true"
                    ...
                    <div class="video-box" v-if="item && item.type == 'video'">
                        <img :src="item.img"
                        @load="imgLoaded(0)"
                        class="video-img"
                        ref="videoImg"
                        ...
                        lazy-load="true"
                        :id="item.id"
                        :style="current == index || true ? videoAnimation : ''"
                        />
                    </div>
                </div>
            </div>
            <div class="water_col">
                <div class="item" ref="rightWaterItem" v-for="(item, index) in rightPosterList" @click="goLink(item.link, `资讯跳转`)">
                    <img :src="item.img"
                    @load="imgLoaded(1)"
                    class="img"
                    lazy-load="true"
                    ...
                    <div class="video-box" v-if="item && item.type == 'video'">
                        ...
                        lazy-load="true"
                        :id="item.id"
                        :style="current == item.id ? videoAnimation : ''"
                        />
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        // 图片加载
        imgLoaded (type) {
            var leftIdx = this.leftPosterList.length - 1
            var rightIdx = this.rightPosterList.length - 1
            if (type) {
                this.rightPosterHeight += this.$refs.rightWaterItem[rightIdx].clientHeight
            } else {
                this.leftPosterHeight += this.$refs.leftWaterItem[leftIdx].clientHeight
            }
            this.posterCount++
            // 加载结束
            if ((this.leftPosterList.length + this.rightPosterList.length) === this.firstPagePoster.firstPagePosterList.length) {
                this.picScroll()
                return
            }
            // 组装左右两列数组
            if (this.leftPosterHeight < this.rightPosterHeight) {
                this.leftPosterList.push(this.firstPagePoster.firstPagePosterList[this.posterCount])
            } else {
                this.rightPosterList.push(this.firstPagePoster.firstPagePosterList[this.posterCount])
            }
        },
        waterFallScroll (e) {
        ...
        picScroll () {
            this.scrollItemList = []
            if (Array.isArray(this.$refs.videoImg)) {
                this.$refs.videoImg.forEach(el => {
                    if (this.isInViewY(el)) {
                        this.scrollItemList.push(el)
                    }
                })
            }
            // else if (this.$refs.videoImg) {
            //     if (this.isInViewY(this.$refs.videoImg)) {
            //         this.scrollItemList.push(el)
            //     }
            // }
            this.scrolling()
        ...
        // 滚动中
        scrolling () {
            this.curVideoImgWidth = this.scrollItemList[0].clientWidth
            this.curVideoImgDuration = 4
            this.current = this.scrollItemList[0].id
            console.log('当前滚动索引', this.current, this.scrollItemList)
            setTimeout(() => {
                if (this.scrollItemList.length === 1) return
                this.scrollItemList.shift()
                this.scrolling()
            }, 4000)
        },
        // 判断元素是否在可视范围内
        isInViewY (element, offset = 0) {
            // 页面滑动的距离
            const pageYScroll = window.pageYOffset || document.documentElement.scrollTop
            // 页面高度
            const pageHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight
            // scrollView
            const scrollY = this.$refs.scrollContainer.scrollTop
            const scrollH = this.$refs.scrollContainer.clientHeight
            const eleRect = element.getBoundingClientRect()

            const wb = pageYScroll + pageHeight
            const et = pageYScroll + eleRect.top
            const eb = et + eleRect.height
            return eb >= pageYScroll - offset && et <= wb + offset
        }
    },
    computed: {
        eyeOpenState () {
            ...
            if (this.firstPageTabsFunds && this.firstPageTabsFunds.tabFunds && this.allFundList) {
                ...
                if (returnData && returnData.length > 0) {
                    returnData.map((item) => {
                        ...
                        // 过滤风险不匹配项
                        if (itemMap.fundCards && itemMap.fundCards && this.userMsg && this.userMsg.riskLevel && !this.showAllFund) {
                            itemMap.fundCards = itemMap.fundCards.filter(
                                i => this.riskLevelOption[i.fundRisk] <= this.userMsg.riskLevel)
                        }
                        itemMap.fundCards = this.$utils.arrSort(itemMap.fundCards, this.sortInfo.key, this.sortInfo.type === 'desc')
                        return itemMap
                    })
                    return returnData
                }
        ...
        videoAnimation () {
            return {
                ...
                transform: `translateX(-${this.curVideoImgWidth - 350}px)`,
            }
        }
    },
    watch: {
        ...
        firstPagePoster (list, oldList) {
            this.leftPosterList.push(list.firstPagePosterList[0])
        },
        isLogin (val, oldval) {
            if (!val && oldval) {
                this.reset()
